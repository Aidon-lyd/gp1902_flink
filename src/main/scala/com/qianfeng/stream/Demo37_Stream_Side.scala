...
object Demo37_Stream_Side {
  def main(args: Array[String]): Unit = {
    ...
    //定义一个侧输流 ---标识
    val outputTag = new OutputTag[(String,String, Long)]("late_stream")

    //3、对source进行transformer
    val res: DataStream[String] = dstream.filter(_.trim.length > 0)
      .map(line => {
      ...
      .assignTimestampsAndWatermarks(new AssignerWithPeriodicWatermarks[(String, String, Long)] {
        ...
        override def extractTimestamp(element: (String, String, Long), previousElementTimestamp: Long): Long = {
          ...
          println(s"Event时间->$now_timestampe | ${fmt.format(now_timestampe)} " +
      ...
      .sideOutputLateData(outputTag)  //加入迟到数据侧输流
